#!/usr/bin/env python3
"""Train XiangqiModelV6 from Pikafish value labels plus a lightweight policy prior."""
...
MODEL_PATH = MODEL_DIR / "xiangqi_model_v6.npz"
REPORT_PATH = MODEL_DIR / "xiangqi_model_v6_report.json"
V5_MODEL_PATH = MODEL_DIR / "xiangqi_model_v5.npz"
V5_REPORT_PATH = MODEL_DIR / "xiangqi_model_v5_report.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Train XiangqiModelV6 from Pikafish labels with a lightweight teacher policy prior."
    )
    ...
    parser.add_argument("--teacher-movetime", type=int, default=30)
    ...
    value_source = "v6_trained"
    if V5_MODEL_PATH.exists() and V5_REPORT_PATH.exists():
        v5_report = json.loads(V5_REPORT_PATH.read_text(encoding="utf-8"))
        v5_val_rmse = float(v5_report.get("val", {}).get("rmse", float("inf")))
        if report["val"]["rmse"] >= v5_val_rmse:
            value_params = load_value_params(V5_MODEL_PATH)
            value_source = "v5_fallback"
            report["value_fallback"] = {
                "used": True,
                "trained_val_rmse": report["val"]["rmse"],
                "fallback_val_rmse": v5_val_rmse,
            }
        else:
            report["value_fallback"] = {
                "used": False,
